...
This is a nice data-science example because the challenge is not just computing p-values. The real challenge is *decision-making under many simultaneous comparisons*.
...
## Key setup: what can happen in a hypothesis test?

Before we compute anything, it helps to be clear about what a hypothesis test is trying to do.

For each gene, we make a decision:

- either *reject* the null hypothesis and call the gene differentially expressed,
- or *do not reject* the null hypothesis and decide that the evidence is not strong enough.

But the gene itself also has a hidden truth:

- either the null hypothesis is really true, meaning the gene is *not* differentially expressed,
- or the null hypothesis is false, meaning the gene *is* differentially expressed.

That gives four possible outcomes.

```{mermaid}
flowchart TD
    A[One gene] --> B{What is true?}
    B --> C[Null true;no real difference]
    B --> D[Null false;real difference]
    C --> E{Our decision}
    D --> F{Our decision}
    E --> G[Reject null;false positive;Type I error]
    E --> H[Do not reject;true negative]
    F --> I[Reject null;true positive]
    F --> J[Do not reject;false negative;Type II error]
```

| Reality | Our decision | Name |
|---|---|---|
| Gene is not differentially expressed | Reject the null | *False positive* = *Type I error* |
| Gene is not differentially expressed | Do not reject the null | *True negative* |
| Gene is differentially expressed | Reject the null | *True positive* |
| Gene is differentially expressed | Do not reject the null | *False negative* = *Type II error* |

For a *single* hypothesis test:

- A *Type I error* means we claim a signal that is not really there.
- A *Type II error* means we miss a real signal.
...
## Key setup: what changes in multiple testing?

In a microarray study, we do not make this decision once. We make it thousands of times, once for each gene.

That means we should keep track not only of individual mistakes, but also of how the *entire collection* of testing results breaks down.

Suppose we test many genes at once. Then the outcomes can be summarized like this:

```{mermaid}
flowchart LR
    A[All tested genes] --> B[Called significant; R = V + S]
    A --> C[Not called significant; m - R = U + T]
    B --> D[True positives;S]
    B --> E[False positives;V]
    C --> F[True negatives;U]
    C --> G[False negatives;T]
```

| Truth about the gene | Called significant | Not called significant |
|---|---|---|
| Null true | *False positive* | *True negative* |
| Null false | *True positive* | *False negative* |

Sometimes statisticians label these counts with letters:
...
From these pieces, a few important totals are built:

- $R = V + S$: total number of genes we call significant,
- $m - R = U + T$: total number of genes we do not call significant.

This notation matters because different statistical goals focus on different parts of the table:

- *Family-wise error rate (FWER)* asks whether we make *any* false positive at all.
- *False discovery rate (FDR)* asks: among the genes we call significant, what fraction are false positives?
- *Power* asks how often we successfully detect truly differentially expressed genes.

The key practical tension is this:

- If we use a very loose threshold, we may get many discoveries, but also many false positives.
- If we use a very strict threshold, we reduce false positives, but we may miss real genes and increase false negatives.

This case study is about learning how to manage that tradeoff in a realistic high-dimensional setting.
...
The paper also argues that large t-statistics do not simply correspond to low-intensity artifacts. This plot helps us see that issue directly.
...
That simple calculation is often the first moment we realize that "p < 0.05" is not remotely enough in a high-dimensional setting.
...
Now we turn to the main goal of this case study: estimating how many of our apparent discoveries may be false.
...
The remaining question at this point is: after all of this adjustment and checking, what genes would we actually carry forward for follow-up?
...
2. Why does permutation help in this problem where the null distribution and dependence structure may be complicated?
