...
- Agresti-Coull interval (adjusted for small samples)
  - $\frac{x + z^2/2}{n + z^2} \pm z \sqrt{\frac{(x + z^2/2)(n - x + z^2/2)}{(n + z^2)^2}}$
...
- Proportions can be thought of as rates where the denominator is fixed (e.g., total population).
- For example, the proportion of people infected in a population can be modeled as a rate of infections per person.
...
- Scenario: An e-commerce company wants to test if a new website design (B) leads to a higher conversion rate than the current design (A).
- Data:
  - Design A: 200 conversions out of 2,000 visitors
  - Design B: 260 conversions out of 2,100 visitors
  - Null hypothesis: The conversion rates for both designs are the same ($p_A = p_B$)
  - Alternative hypothesis: Design B has a higher conversion rate than Design A ($p_B > p_A$)
...
**Output:** (-2.41, 0.016) → We reject the null hypothesis at the 5% significance level, suggesting that design B has a significantly higher conversion rate than design A. This does not mean that we accept the alternative hypothesis with certainty, but it provides evidence in favor of it. 

In other words: we are 95% confident that the true conversion rate for design B is higher than that for design A.
...
ci_low, ci_high = smp.proportion_confint(count[1] - count[0], nobs[1] + nobs[0], method="wilson")
(ci_low, ci_high)
```

**Output:** (0.011, 0.018) → The 95% confidence interval for the difference in conversion rates between design B and design A is between 1.1% and 1.8%, suggesting that design B's conversion rate is likely higher than design A's by this margin. Because the interval does not include 0, it supports the conclusion that there is a significant difference in conversion rates between the two designs.
...
- Data:
  - Before: 18 accidents over 50,000 hours
...
**Output:** (1.98, 0.048) → The test statistic is 1.98 with a p-value of 0.048, indicating a statistically significant difference in rates at the 5% level, with the after-program rate being lower.
...
**Output:** (0.45, 0.20, 1.01) → The rate ratio is 0.45, suggesting that the accident rate after the program is 55% lower than before. The 95% confidence interval for the rate ratio ranges from 0.20 to 1.01. This interval includes 1, which means that while the point estimate suggests a reduction in accident rates, we cannot rule out the possibility that there is no effect (rate ratio of 1) at the 5% significance level. However, the p-value from the previous test indicates that there is evidence to suggest a significant reduction in accident rates after the program.
...
- Not checking assumptions (independence, overdispersion)
