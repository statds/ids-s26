...
* **Gradient descent**: an optimization algorithm that minimizes the loss function (prediction error) by *fitting each new model to the previous model's residuals* (i.e. target is the residuals, data is )
...
We create a model using a training set where the initial **prediction** of the target variable is usually the variable's mean.
...
The **residuals of the previous model become the target variable** in the new model, keeping features the same. Usually, this model is a decision tree.
