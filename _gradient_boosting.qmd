...
![Gradient Descent Visual Representation](https://datamites.com/blog/uploads/images/202403/image_750x_660501cb801fd.jpg)

![Learning Rate Visual Representation](https://datahacker.rs/wp-content/uploads/2021/10/116-1920x549.jpg)
...
or
$$
r_{im} = y_{i} - \hat{y}_{i}
$$

**which is just the usual residual calculation (actual - predicted)** where:
...
We want to create a model that is able to predict the median house values for California districts.
...
**To see improvement**, you may want to try the `GridSearchCV` function in order to find which combinations of hyperparameters yield the best results (however this is very computationally expensive, especially considering how many hyperparameters Gradient Boosting has).
...
We want to create a model that is able to predict whether a patient's tumor is malignant or benign.
...
print(f'Accuracy score: {gb_acc:.2f}')
```
