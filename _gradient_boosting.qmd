...
The **pseudo-residuals of the previous model, which are the negative gradients (when the loss function is MSE), become the target variable** in the new model. The goal of this model is to **predict the errors of the previous model.**

"Following" the negative gradients down the loss function by using them as targets to be predicted leads to the minimization of loss as the number of models grows. This is how each new model learns to fix the errors of the previous model. 

Usually, this model is a decision tree because they're fairly versatile.

To help with overfitting (which is something Gradient Boosting is prone to), each model's predictions are scaled according to the learning rate (a process called shrinkage), which is what weights each model's contribution to the final prediction.
...
Finally, the predictions of all models in the ensemble are combined. This is what the final prediction of the initial target variable looks like:

$$
y_{pred} = initial prediction + \eta \times r_{1} + \eta \times r_{2} + ... \eta \times r_{n}
$$
...
* $r_{n}$ is the residuals of the nth model.
